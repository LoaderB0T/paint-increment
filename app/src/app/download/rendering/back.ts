...
export const renderBack = async (lobby: LobbyResponse, color: string, transparent: boolean, columns: number) => {
  ...
  drawSquaresForIterations(lobby, columns, borderThickness, availableSizePerIteration, textSpace, transparent, ctx);
  clearRowsForText(rows, borderThickness, availableSizePerIteration, textSpace, transparent, ctx, actualCanvasWidth);
  ...
  for (let i = 0; i < lobby.pixelIterations.length; i++) {
    ...
    drawContributorName(ctx, contributor, startX, availableSizePerIteration, startY, borderThickness, textSpace);
